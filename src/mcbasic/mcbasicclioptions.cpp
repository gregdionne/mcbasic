...
MCBASICCLIOptions::MCBASICCLIOptions()
    : native{false, "native", nullptr,
             ...
             " Many programs are too big to use native."},
      g{false, "g", nullptr,
      ...
      list{
          ...
          "as a replacement for IF..GOTO/GOSUB ELSE).  These extensions are "
          "internal and are not available for use in the source BASIC files."},
      el{false, "el", nullptr,
         ...
         "0, 0, 0, 400, and 0 for values of X from 1-9, respectively; and to "
         "line 500 for other values of X."},
      ul{false, "ul", nullptr,
         "Do not generate compilation errors on unlisted line numbers.",
         "A runtime '?UL ERROR' will be generated instead.  This is intended "
         "to be used in conjunction with the '-el' compiler flag.  Use '-g' to "
      ...
      mcode{
          ...
          "MICROCOLOR BASIC interpreter is running or modify direct page "
          "variables needed by the compiler to function.  Sic hunt dracones!"},
      undoc{false, "undoc", nullptr,
            "Enable compilation with undocumented opcodes.",
            "Currently only the undocumented negate-with-carry instructions.  "
            "The MC6801 and MC6809 both implement 'NGCA' (opcode $42), 'NGCB' "
            "(opcode $52), 'NGC <indexed>' (opcode $62), and 'NGC <extended>' "
            "(opcode $72).  These permit negation of the 'D' register in four "
            "cycles by issuing a 'NEGB' (opcode $50) followed by 'NGCA' "
            "(opcode $42).  They can also negate multi-byte numbers in-place.  "
            "Use caution when using the undoc switch, as many 6801 emulators "
            "fail to implement these instructions correctly."},
      S{false, "S", nullptr,
        "Generate source without assembling to machine code.",
        "Compiles, but does not assemble the named BASIC files.  The "
        "assembly language output is left in the corresponding .asm files.\n\n"
        "The source files are compatible with the Telemark assembler that was "
        "bundled with the Virtual MC-10.  You can also use the version "
        "found at: https://github.com/gregdionne/tasm6801.  Otherwise, you "
        "can compile the generated .asm file by passing it as a filename to "
        "\b."},
      c{false, "c", nullptr,
        "Generate object code without generating cassette file.",
        "Compiles the named BASIC files.  The binary output is left "
        "in the corresponding .obj files, which can be loaded into "
        "the Virtual MC-10 via the 'Load Binary File' in the 'Util' menu.  "
        "Set the load and EXEC addresses to $4346.  You can then EXEC the "
        "program as if it was loaded via a .c10 file."},
      Wfloat{
          ...
          "that can be proven to be of pure integral type.  The remainder of "
          "the variables are then promoted to floating point.  The current "
          "backend implements integral variables with three bytes (24 bit "
          "two's complement) and, in turn, emulates floating point with five "
          "byte fixed-point (24 bit two's complement integer; 16-bit "
          "fraction).  While that makes it faster than floating-point; it's "
          "far less accurate, takes more memory, and is slower than using "
          "integers.  Avoid floating-point if you can."},
      ...
      Wuninit{true, "Wuninit", "Wno-uninit",
              "Warn when a variable is used but never initialized anywhere.",
              "The compiler will replace references to uninitialized string "
              "variables with \"\" and uninitialized numeric variables with 0 "
              "and remove the variable from the program.  This is likely to be "
              "the result of a typo or a forgotten initialization."},
      Wunused{
          true, "Wunused", "Wno-unused",
          "Warn when a variable is assigned to an expression but never used.",
          "The compiler deletes these variables from the program.  If the "
          "expression invokes RND or INKEY$, a special statement, 'EVAL' is "
          "used to ensure that the side-effects of changing the random number "
          "seed or adjusting the keyboard registers is performed.  Otherwise "
          "the expression will be deleted as well.  The warning usually occurs "
          "as the result of a typo or forgetting to use the variable."},
