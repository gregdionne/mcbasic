0 CLEAR500:POKE16925,0:POKE16926,1:GOSUB2000:GOTO100
1 RETURN
2 PRINT@A(X,Y),B$;:PRINT@B(X,Y),B$;:RETURN
3 PRINT@A(X,Y),A$(D);:PRINT@B(X,Y),B$(D);:RETURN
4 ON-(X<2)GOTO40:ONK(PEEK(A(X-2,Y)+M))GOTO40,40,42,40:GOTO42
5 ON-(X>13)GOTO50:ONK(PEEK(A(X+2,Y)+M))GOTO50,50,52,50:GOTO52
6 ON-(Y<2)GOTO60:ONK(PEEK(A(X,Y-2)+M))GOTO60,60,62,60:GOTO62
7 ON-(Y>5)GOTO70:ONK(PEEK(A(X,Y+2)+M))GOTO70,70,72,70:GOTO72
8 ONK(PEEK(A(X-1,Y)+M))GOTO4,90,80,1:PRINT@A(X,Y),B$;:PRINT@B(X,Y),B$;:X=X-1:D=6:GOTO3
9 ONK(PEEK(A(X+1,Y)+M))GOTO5,91,81,1:PRINT@A(X,Y),B$;:PRINT@B(X,Y),B$;:X=X+1:D=5:GOTO3
10 ONK(PEEK(A(X,Y-1)+M))GOTO6,92,82,1:PRINT@A(X,Y),B$;:PRINT@B(X,Y),B$;:Y=Y-1:D=7:GOTO3
11 ONK(PEEK(A(X,Y+1)+M))GOTO7,93,83,1:PRINT@A(X,Y),B$;:PRINT@B(X,Y),B$;:Y=Y+1:D=7:GOTO3
...
20 FORT=1TOE:FORJ=1TOL:ONRND(G(J))GOSUB12,13:FORZZ=1TO300:NEXT:IFPEEK(2)=.THENNEXT:NEXT:GOTO26
21 IF1ANDNOTPEEK(16946)THENIFX>.THENGOSUB8:NEXT:NEXT:GOTO26
22 IF1ANDNOTPEEK(16949)THENIFX<15THENGOSUB9:NEXT:NEXT:GOTO26
23 IF4ANDNOTPEEK(16952)THENIFY>.THENGOSUB10:NEXT:NEXT:GOTO26
24 IF4ANDNOTPEEK(16948)THENIFY<7THENGOSUB11:NEXT:NEXT:GOTO26
25 NEXT:NEXT
26 ONT-E+ZGOTO30,94,95,97,97,98
...
42 H=.:FORC=X-1TO1STEP-1:ONK(PEEK(A(C-1,Y)+M))GOTO49,49,46,49:PRINT@A(C,Y),B$;:PRINT@B(C,Y),B$;
...
52 H=.:FORC=X+1TO14:ONK(PEEK(A(C+1,Y)+M))GOTO59,59,56,59:PRINT@A(C,Y),B$;:PRINT@B(C,Y),B$;
...
57 FORJ=1TO4:ON1-(C+1=M(J)ANDY=N(J))GOTO58:M(J)=-1:G(J)=.:H=H+1:I=I+1:J=4:NEXT:GOTO54
58 NEXT:STOP
59 C=14:NEXT:GOTO88
...
67 FORJ=1TO4:IFX=M(J)ANDC-1=N(J)THENM(J)=-1:G(J)=.:H=H+1:I=I+1:J=4:NEXT:GOTO64
68 NEXT:STOP
69 C=1:NEXT:GOTO88
...
77 FORJ=1TO4:IFX=M(J)ANDC+1=N(J)THENM(J)=-1:G(J)=.:H=H+1:I=I+1:J=4:NEXT:GOTO74
78 NEXT:STOP
79 C=6:NEXT:GOTO88
...
84 FORT=1TO5:D=7:GOSUB3:SOUND200,5:D=8:GOSUB3:SOUND1,3:NEXT:D=1:GOSUB3:T=E+2:J=4:K=.:RETURN
...
285 GOTO260
290 XX=1:YY=.:IFX<14THENIFK(PEEK(A(X+XX+XX,Y)+M))=1THENFORC=1TO2:PRINT@A(X+(XX*C),Y),B$;:PRINT@B(X+(XX*C),Y),B$;:NEXT:GOTO330
295 GOTO260
300 XX=-1:YY=.:IFX>1THENIFK(PEEK(A(X+XX+XX,Y)+M))=1THENFORC=1TO2:PRINT@A(X+(XX*C),Y),B$;:PRINT@B(X+(XX*C),Y),B$;:NEXT:GOTO330
305 GOTO260
330 X=X+(XX*2):Y=Y+(YY*2):X(K)=X:Y(K)=Y:K=K+1:GOTO210
350 K=K-1:X=X(K):Y=Y(K)
360 IFK=0THEN400
370 GOTO210
400 K=17023:Y=7:FORT=1TO20+LV:X=RND(14)-1:Y=RND(7):GOSUB2:NEXT:X=15:FORY=1TO6:ONRND(2)GOSUB2:NEXT:RETURN
...
595 GOSUB3:RETURN
600 DATA 170,2,170,2,176,2,185,2,185,2,176,2,170,2,159,2,147,2
610 DATA 147,2,159,2,170,2,170,3,159,1,159,4
620 DATA 170,2,170,2,176,2,185,2,185,2,176,2,170,2,159,2,147,2
630 DATA 147,2,159,2,170,2,159,3,147,1,147,4,-1,-1
640 DATA 159,2,159,2,170,2,147,2,159,2,170,1,176,1,170,2,147,2
650 DATA 159,2,170,1,176,1,170,2,159,2,147,2,159,2,108,4
660 DATA 170,2,170,2,176,2,185,2,185,2,176,2,170,2,159,2,147,2
670 DATA 147,2,159,2,170,2,159,3,147,1,147,4,-1,-1
700 READA,B
...
1410 FORX=2TO17:FORY=2TO9:L(X,Y)=2:NEXT:NEXT:FORX=1TO10:NM$(X)="---":NEXT
1500 RETURN
2000 CLS
2001 PRINT"    PENGUINO    BY JIM GERRIE"
2005 PRINT"      ÀÀÀÀ      RETROSPECTIVA"
2010 PRINT"     ÀÀÀÀÀÀ     BASIC GAME"
2020 PRINT"    ÏÏÏÀÀÏÏÏ    CONTEST 2012"
2030 PRINT"    ÏÀÏÀÀÏÀÏ       
2040 PRINT"    ÏÏÏÀÿÏÏÏ    SQUASH"
2050 PRINT"     ÏÏÏÏÏÏ     SNOW-BEES 100PTS";
2060 PRINT"     ¿ÏÏÏÏ¿     BY PUSHING
...
3120 PRINT@32*14+16,"¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯";
3200 FORA=1TO10:PRINT@32*(3+A)+17,MID$(STR$(A)+"  ",1,4)NM$(A)HS(A);:NEXT
3210 RETURN
4000 IFSC>HS(10)THEN4050
4020 RETURN
4050 CLS:PRINT"YOU MADE THE TOP 10!"
4060 INPUT"INITIALS (MAX 3)";TM$:NM$=LEFT$(TM$+"   ",3):IFNM$="ASS"ORNM$="FUC"ORNM$="BUM"ORNM$="CFG"THENNM$="!@#"
