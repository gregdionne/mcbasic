...
13 RESET(L,M):ONPOINT(L+H,M+I)GOTO36,30,32,32,32,32,36:ONPOINT(L+N,M+O)GOTO36,30,32,32,32,32,36:L=L+N:M=M+O:SET(L,M,7):RETURN
...
16 IFR>1THENGOSUB12
17 FORZ2=1TO2:IFLTHENGOSUB13
18 NEXT:FORZ2=1TOZ3:NEXT:GOSUB300:IFJ=9THENGOSUB40:NEXT:NEXT:GOTO48
...
51 IFEV=1THEN?@480,"LEVEL CLEARED.";:FORT=1TO10:SOUND100,1:SOUND200,1:NEXT:SC=SC+250:HT=0
...
62 CLS0:LV=5:LF=2:M(1)=9:HT=0:EN=4:EX=RND(3):IFSC>HSTHENHS=SC
63 SC=.:GOTO73
64 DIMA$(13),B$(13),X(6),Y(6),R(6),O(2),P(2),K(255),A(31,14),B(31,14),E(64),F(64),G(64),I(12,1),J(6,1)
65 DIMT,D,X,Y,A,B,C,E,F,H,I,G,R,L,S,M,W,J,K,N,O,Q,U,V,Z,B$,M(12),N(8),Q(8),L(8),DR(4,6,1),EX(3):POKE16925,0:POKE16926,1
66 DIMLV,EV,SC,EN,EX,BN,HS,XX,YY,CX,CY,XC,YC,PC,RR,HT,I$,A$:B$=CHR$(128)+CHR$(128):LV=8:LF=3:GOSUB103:GOSUB96
67 GOSUB162:FORT=.TO13:FORY=.TO3:READA$:FORX=1TO4:I=VAL(MID$(A$,X,1)):IFI>.THENSET(X-1,Y,I)
68 NEXT:NEXT:A$(T)=CHR$(PEEK(16384))+CHR$(PEEK(16384+1)):B$(T)=CHR$(PEEK(16384+32))+CHR$(PEEK(16384+33))
69 GOSUB99:NEXT:GOSUB400:GOSUB117:GOSUB100:W=-1:O(0)=-1:O(1)=3:O(2)=5:P(0)=-1:P(1)=1:P(2)=5:I=.:GOSUB161
70 FORT=1TO12:READC1:I(T,0)=INT(C1):NEXT:FORT=1TO6:READC1:J(T,0)=INT(C1):NEXT:FOREN=1TO4
71 FORT=1TO6:READC1:DR(EN,T,0)=INT(C1):READC1:DR(EN,T,1)=INT(C1):NEXT:NEXT:EN=4:EX=RND(3)
72 FORT=1TO12:READC1:M(T)=INT(C1):NEXT
73 IFM(1)=9THENLV=LV+3:LF=LF+1:RESTORE:FORT=1TO(14*4):READA$:NEXT:FORT=1TO12+6+(4*12):READC1:C=INT(C1):NEXT:GOSUB151:GOTO72
...
97 FORX=0TO64:E(X)=X*2+5:F(X)=X*2+1:G(X)=X*2-1:NEXT:RETURN
99 PRINT@0,B$;:PRINT@32,B$;:RETURN
100 PRINT@483,"PRESS ANY KEY TO CONTINUE";
...
132 ?"GET EXTRA HEALTH EVERY LEVEL"
133 ?"CLEARED AND A LIFE EVERY 10."
134 RETURN
...
162 K(76)=1:K(83)=2:K(88)=3:K(74)=4:K(65)=5:K(90)=6:K(75)=7:K(73)=8:K(32)=9
163 N(1)=1:Q(1)=.:L(1)=3:N(2)=1:Q(2)=-1:L(2)=4:N(3)=1:Q(3)=1:L(3)=5:N(4)=-1:Q(4)=.:L(4)=6
164 N(5)=-1:Q(5)=-1:L(5)=7:N(6)=-1:Q(6)=1:L(6)=8:N(7)=.:Q(7)=1:L(7)=9:N(8)=.:Q(8)=-1:L(8)=9:RETURN
169 DATA "...."
170 DATA "...."
171 DATA "...."
172 DATA "...."
...
300 J=0:IFPEEK(2)AND8ANDNOTPEEK(16952)THENJ=K(32):RETURN
310 IFPEEK(2)AND2ANDNOTPEEK(16946)THENJ=K(73):RETURN
320 IFPEEK(2)AND2ANDNOTPEEK(16947)THENJ=K(74):RETURN
330 IFPEEK(2)AND2ANDNOTPEEK(16948)THENJ=K(75):RETURN
340 IFPEEK(2)AND2ANDNOTPEEK(16949)THENJ=K(76):RETURN
350 IFPEEK(2)AND1ANDNOTPEEK(16946)THENJ=K(65):RETURN
360 IFPEEK(2)AND4ANDNOTPEEK(16948)THENJ=K(83):RETURN
370 IFPEEK(2)AND8ANDNOTPEEK(16947)THENJ=K(90):RETURN
380 IFPEEK(2)AND8ANDNOTPEEK(16945)THENJ=K(88):RETURN
390 RETURN
400 PRINT@482,"SELECT LEVEL OF PLAY (1-3)?";
401 I$=INKEY$:IFI$<"1"ORI$>"3"THEN401
402 Z3=550-INT(VAL(I$))*50:RETURN
