...
250 PRINT@31,"$";:ONA(X,Y)GOTO3000,480,3000,380,330,3000,280,250,330
255 REM KING MOVE
260 FORA=X-1TOX+1:FORB=Y-1TOY+1:IFA(A,B)<>0THENGOSUB650:NEXT:NEXT:RETURN
261 NEXT:NEXT:RETURN
265 REM PLAYER PIECE MOVE
270 ON-A(X,Y)GOTO3000,530,3000,380,330,3000,280,3000,330:GOTO260
...
920 IFI*A(X,Y)>0THENONABS(A(X,Y))GOTO960,970,3000,980,990,3000,1000,3000,1010:PRINT@L," K ";:RETURN
921 ONABS(A(X,Y))GOTO960,1040,3000,1050,1060,3000,1070,3000,1080:PRINT@L," k ";:RETURN
940 IFI*A(X,Y)>0THENONABS(A(X,Y))GOTO1100,1110,3000,1120,1130,3000,1140,3000,1150:PRINT@L,CHR$(128);"K";CHR$(128);:RETURN
941 ONABS(A(X,Y))GOTO1100,1180,3000,1190,1200,3000,1210,3000,1220:PRINT@L,CHR$(128);"k";CHR$(128);:RETURN
960 PRINT@L,"   ";:RETURN
